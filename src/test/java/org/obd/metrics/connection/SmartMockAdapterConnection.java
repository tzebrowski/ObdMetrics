...
public final class SmartMockAdapterConnection implements AdapterConnection {
	...
	public static AdapterConnection get(final PIDsRegistry registry, final Adjustments optional,
			final Query query, final Init init, Strategy strategy) {
		...
		commandsSuplier.get().forEach(e -> {
			...
			builder.requestResponse(ecuQuery, multiFrameGenerator.generateAnswers(ecuQuery, 5, policy));
		});
    ...
    @Builder
    public static SmartMockAdapterConnection build(@Singular("requestResponse") Map<String, List<String>> requestResponse,
            long writeTimeout,
